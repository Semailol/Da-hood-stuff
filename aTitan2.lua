	...
	if LocalPlayer.Name == MyStand then
		if XD == false then
			...
			elseif i.KeyCode == Enum.KeyCode.Q and LocalPlayer:GetMouse().Target then
				play(7267875088, true, true)
				LOLXD()
				LocalPlayer.Character:FindFirstChild("HumanoidRootPart").CFrame = CFrame.new(LocalPlayer:GetMouse().Hit.x, LocalPlayer:GetMouse().Hit.y + 5,LocalPlayer:GetMouse().Hit.z)
...
LocalPlayer.Chatted:Connect(function(MSG)
	if LocalPlayer.Name == MyStand then
		...
		elseif MSG == Transform and LOL == false then
			...
			LocalPlayer.Character.HumanoidRootPart.CFrame = game.Workspace.Ignored.Shop["[High-Medium Armor] - $2300"].Head.CFrame + Vector3.new(0, 3, 0)
			if (LocalPlayer.Character.HumanoidRootPart.Position - game.Workspace.Ignored.Shop["[High-Medium Armor] - $2300"].Head.Position).Magnitude <= 50 then
				wait(.2)
				fireclickdetector(game.Workspace.Ignored.Shop["[High-Medium Armor] - $2300"].ClickDetector, 4)
				wait()
