...
uis.InputBegan:Connect(function(i, XD)
	if i.KeyCode == Enum.KeyCode.T and XD == false then
	...
	if XDR == false then
		...
		for i, v in pairs(game.Workspace.MAP:GetDescendants()) do
			if v:IsA("BasePart") or v:IsA("WedgePart") or v:IsA("Part") or v:IsA("BallPart") or v:IsA("MeshPart") or v:IsA("Decal") or v:IsA("Texture") then
				v.Transparency = v.Transparency+0
			end
		end
	else
		XD = false
...
function Jump(...)
	(...).Jumping:Connect(function()
		...
		if CD == false and not Poter.IsPlaying then
			...
			AddVelocity(OldChar.HumanoidRootPart.CFrame.UpVector*50, OldChar)
			play(0, true, true)
			delay(0, function()
				IsJump = true
				wait(1) 
...
rm();
wait(0.5);
game:GetService('Players').LocalPlayer.Character:FindFirstChildOfClass('Humanoid'):FindFirstChild('BodyTypeScale'):Destroy();
wait(1);

rm();
wait(0.5);
game:GetService('Players').LocalPlayer.Character:FindFirstChildOfClass('Humanoid'):FindFirstChild('BodyWidthScale'):Destroy();
wait(1);

rm();
wait(0.5);
game:GetService('Players').LocalPlayer.Character:FindFirstChildOfClass('Humanoid'):FindFirstChild('BodyDepthScale'):Destroy();
wait(1);

rm();
wait(0.5);
game:GetService('Players').LocalPlayer.Character:FindFirstChildOfClass('Humanoid'):FindFirstChild('HeadScale'):Destroy();
wait(1);
    end
end)
